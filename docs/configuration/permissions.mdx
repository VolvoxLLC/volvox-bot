...
<Callout type="info">
Global admin status is independent of any server's roles. A user listed in `BOT_OWNER_IDS` only gains access to the operator-only pages above — normal Discord permissions still apply for the servers they belong to.
</Callout>
