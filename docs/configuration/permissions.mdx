...
Each ID must be a Discord user ID (snowflake) for an account that logs in to the dashboard with Discord OAuth. Changes take effect after you restart every process or container that reads `BOT_OWNER_IDS`, including both the bot and dashboard when they run separately.

<Callout type="info">
Global admin status is independent of any server's roles. A user listed in `BOT_OWNER_IDS` only gains access to the operator-only pages above — normal Discord permissions still govern what they can do in the servers they belong to.
</Callout>
