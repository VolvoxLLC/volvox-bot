...
The audit log records every triggered AI moderation response as `ai_automod.<action>` for each action that actually runs. It records `ai_automod.none` only when a threshold is hit but no action runs. Global instant enforcement (`autoDelete`) or an incident report channel (`flagChannelId`) can still trigger delete or flag actions, even if you haven't selected a category-specific response.

Triage model settings are in **Config → AI & Automation → Triage**. Triage uses the same supported model dropdown as Content Safety for both classification and responses — you can't enter a custom model string.
