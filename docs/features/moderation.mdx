...
Triage model settings are in **Config → AI & Automation → Triage**. The classifier and response engines use the same supported model dropdown as Content Safety. If your saved config contains an invalid or unsupported model string, the bot logs a warning, tries supported configured values in the fixed precedence order, and falls back to the default AI model if none are supported.
