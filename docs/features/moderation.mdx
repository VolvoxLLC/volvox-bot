...
Triage model settings are in **Config → AI & Automation → Triage**. The classifier and response engines use the same supported model dropdown as Content Safety; if saved config contains an invalid or unsupported model string, the bot logs a warning and falls back through the configured precedence chain, then uses the default model if no configured value is supported.
