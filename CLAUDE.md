...
- Security hardening for CI review workflow:
  - Removed dynamic Claude plugin marketplace loading from `.github/workflows/claude-review.yml` (`plugin_marketplaces` and `plugins`).
  - Updated the workflow prompt to use built-in review behavior directly instead of `/code-review` plugin command.
  - Reason: prevent unpinned third-party plugin code from executing in a job with write permissions and secrets.
