...
- Edit only `coverage-exclusions.json`, then run `pnpm sonar:sync-coverage-exclusions`.
- CI runs `pnpm sonar:check-coverage-exclusions`, which fails if Sonar drifts or Vitest stops consuming the JSON directly.
