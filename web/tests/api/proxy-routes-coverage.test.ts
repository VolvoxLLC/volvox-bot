...
function expectSearchParams(url: URL, expected: Record<string, string>) {
  for (const [key, value] of Object.entries(expected)) {
    expect(url.searchParams.get(key)).toBe(value);
  }
}

async function expectProxiedRoute(routeCase: ProxyRouteCase) {
...
async function expectProxiedRoutes(cases: readonly ProxyRouteCase[]) {
  for (const routeCase of cases) {
    await expectProxiedRoute(routeCase);
  }
}

function proxyCases(cases: readonly ProxyRouteCase[]): readonly ProxyRouteCase[] {
  return cases;
}

async function expectSharedProxyFailures(
  call: () => Promise<Response>,
