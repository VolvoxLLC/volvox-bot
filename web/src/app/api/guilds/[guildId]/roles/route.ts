...
export async function GET(
  ...
  try {
    ...
    logger.error(
      "[api/guilds/:guildId/roles] Failed to verify guild permissions:",
      error,
    );
    return NextResponse.json(
  ...
  if (!botApiBaseUrl || !botApiSecret) {
    logger.error(
      "[api/guilds/:guildId/roles] BOT_API_URL and BOT_API_SECRET are required",
    );
    return NextResponse.json(
  ...
  try {
    upstreamUrl = new URL(
      `${botApiBaseUrl}/guilds/${encodeURIComponent(guildId)}/roles`,
    );
  } catch {
    logger.error("[api/guilds/:guildId/roles] Invalid BOT_API_URL", {
      botApiBaseUrl,
    });
    return NextResponse.json(
  ...
  try {
    ...
    logger.error("[api/guilds/:guildId/roles] Failed to proxy roles:", error);
    return NextResponse.json(
