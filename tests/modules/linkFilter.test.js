...
describe('checkLinks — edge cases', () => {
  ...
  it('blocks domain regardless of case in URL', async () => {
    const config = makeConfig({ blockedDomains: ['evil.com'] });
    const msg = makeMessage({ content: 'Visit https://EVIL.COM/page' });

    const result = await checkLinks(msg, config);
    expect(result.blocked).toBe(true);
  });

  it('handles mixed case in blocklist entry', async () => {
    // Blocklist domains should be lowercase for matching to work
    const config = makeConfig({ blockedDomains: ['evil.com'] });
    const msg = makeMessage({ content: 'check https://EVIL.COM' });

    // Domain extraction lowercases URLs, so this should match
    const result = await checkLinks(msg, config);
    expect(result.blocked).toBe(true);
  });

  it('blocks multiple nested subdomains correctly', async () => {
    const config = makeConfig({ blockedDomains: ['evil.com'] });
    const msg = makeMessage({ content: 'https://deep.nested.sub.evil.com/path' });
  ...
  it('does not match partial domain names', async () => {
    // Regression test: ensure "evil.com" doesn't match "notevil.com"
    const config = makeConfig({ blockedDomains: ['evil.com'] });
    const msg = makeMessage({ content: 'https://notevil.com/safe' });
  ...
  it('handles URLs with query parameters and fragments', async () => {
    const config = makeConfig({ blockedDomains: ['bad.io'] });
    const msg = makeMessage({ content: 'https://bad.io/page?param=value#section' });

    const result = await checkLinks(msg, config);
    expect(result.blocked).toBe(true);
  });

  it('detects phishing patterns with mixed case keywords', async () => {
    const config = makeConfig({ blockedDomains: [] });
    const msg = makeMessage({ content: 'FREE at https://DISCORD-nitro.xyz/claim' });
