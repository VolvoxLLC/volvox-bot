...
// ── Text accumulation from assistant messages (short-lived mode) ─────────────

describe('CLIProcess — short-lived text accumulation', () => {
  beforeEach(() => {
    spawnMock = vi.fn(() => createFakeProc());
    lastSpawnedProc = undefined;
  });
  ...
  it('should reconstruct result.result from accumulated assistant text blocks', async () => {
    const cli = new CLIProcess('test', {});
    await cli.start();
    ...
    // Simulate two assistant messages followed by a result with no result field
    writeLine(proc, {
      type: 'assistant',
    ...
    // result message without .result field — should be reconstructed from accumulated text
    writeLine(proc, { type: 'result', is_error: false });
    await tick();
    proc.emit('exit', 0, null);
  ...
  it('should use result.result when present even if assistant messages exist', async () => {
    const cli = new CLIProcess('test', {});
    await cli.start();
    ...
    // result message WITH an explicit .result field
    writeLine(proc, { type: 'result', result: 'explicit result', is_error: false });
    await tick();
    proc.emit('exit', 0, null);

    const result = await sendP;
    // When result.result is already defined, accumulated text is not used
    expect(result.result).toBe('explicit result');
  });

  it('should ignore assistant message blocks without text type', async () => {
    const cli = new CLIProcess('test', {});
    await cli.start();
    ...
    // Block with type 'tool_use' should NOT be accumulated
    writeLine(proc, {
      type: 'assistant',
      message: { content: [{ type: 'tool_use', id: 'tu_1', name: 'WebSearch' }] },
    });
    writeLine(proc, { type: 'result', is_error: false });
    await tick();
    proc.emit('exit', 0, null);
    ...
    const result = await sendP;
    // result.result should be undefined because no text blocks were accumulated
    expect(result.result).toBeUndefined();
  });
});

// ── buildEnv additional coverage ─────────────────────────────────────────────

describe('CLIProcess — buildEnv additional coverage', () => {
  beforeEach(() => {
    spawnMock = vi.fn(() => createFakeProc());
    lastSpawnedProc = undefined;
  });
  ...
  it('should forward DISABLE_PROMPT_CACHING env var when set', async () => {
    const orig = process.env.DISABLE_PROMPT_CACHING;
    ...
    try {
      ...
      const [, , opts] = spawnMock.mock.calls[0];
      expect(opts.env.DISABLE_PROMPT_CACHING).toBe('1');
      ...
      if (orig === undefined) {
        ...
        process.env.DISABLE_PROMPT_CACHING = orig;
      }
    }
  });

  it('should NOT include DISABLE_PROMPT_CACHING when it is not set', async () => {
    const orig = process.env.DISABLE_PROMPT_CACHING;
    delete process.env.DISABLE_PROMPT_CACHING;
    ...
    try {
      ...
      const [, , opts] = spawnMock.mock.calls[0];
      expect(opts.env.DISABLE_PROMPT_CACHING).toBeUndefined();
  ...
  it('should reject malformed baseUrl and not set ANTHROPIC_BASE_URL', async () => {
    const cli = new CLIProcess('test', { baseUrl: 'not-a-valid-url' });
    await cli.start();
  ...
  it('should reject non-https/http baseUrl (e.g. ftp:// scheme) and not set ANTHROPIC_BASE_URL', async () => {
    const cli = new CLIProcess('test', { baseUrl: 'ftp://proxy.example.com/path' });
    await cli.start();
...
// ── buildArgs additional flags ────────────────────────────────────────────────

describe('CLIProcess — buildArgs additional flags', () => {
  beforeEach(() => {
    spawnMock = vi.fn(() => createFakeProc());
    lastSpawnedProc = undefined;
  });
  ...
  it('should include --system-prompt flag when systemPrompt is set', async () => {
    const cli = new CLIProcess('test', { systemPrompt: 'You are helpful.' });
    await cli.start();
  ...
  it('should include --append-system-prompt flag when appendSystemPrompt is set', async () => {
    const cli = new CLIProcess('test', { appendSystemPrompt: 'Always be concise.' });
    await cli.start();
  ...
  it('should include --tools flag when tools is set (empty string disables tools)', async () => {
    const cli = new CLIProcess('test', { tools: '' });
    await cli.start();
  ...
  it('should use custom permissionMode when provided', async () => {
    const cli = new CLIProcess('test', { permissionMode: 'default' });
    await cli.start();
  ...
  it('should include allowedTools as single --allowedTools arg when string', async () => {
    const cli = new CLIProcess('test', { allowedTools: 'WebSearch' });
    await cli.start();
    ...
    expect(args[atIdx + 1]).toBe('WebSearch');
  ...
  it('should include --no-session-persistence in args', async () => {
    const cli = new CLIProcess('test', {});
    await cli.start();
